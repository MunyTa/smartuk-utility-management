...
Сейчас endpoint предназначен для локального Docker-стенда. Для промышленного запуска его нужно закрыть внутренним API-ключом или сетевым доступом только из контейнера симулятора.
